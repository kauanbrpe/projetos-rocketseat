            ...
            <DialogContent>
                <form onSubmit={form.handleSubmit(handleSubmit)}>
                    ...
                    <DialogBody className="flex flex-col gap-5">
                        ...
                        <InputSingleFile form={form} allowedExtensions={['png', 'jpg', 'jpeg']} maxFileSizeInMB={50} replaceBy={
                            <ImagePreview src={fileScource} className="w-full h-56" />
                            }
                        ...
                        <div className="space-y-3">
                            ...
                            {!isLoadingAlbums && albums.length > 0 && albums.map(album =>
                                <Button key={album.id} variant={albumsIds?.includes(album.id) ? "primary" : "ghost"} size="sm" className="truncate" onClick={() => handleToggleAlbum(album.id)}>
                                    {album.title}
                                </Button>
                            )}

                            {isLoadingAlbums && Array.from({length: 5}).map((_, index) => <Skeleton key={`album-loading-${index}`} className="w-20 h-7"/>)}
                            </div>
                        </div>
                    </DialogBody>
