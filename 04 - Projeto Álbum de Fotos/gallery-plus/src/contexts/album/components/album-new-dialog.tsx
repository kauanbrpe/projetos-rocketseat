...
import SelectCheckedIllustrantion from "../../../assets/images/select-checkbox.svg?react"
                    ...
                    <div className="space-y-3">
                        ...
                        {!isLoadingPhotos && photos.length > 0 &&
                            <div className="flex flex-wrap gap-2">
                                {photos.map((photo) => <PhotoImageSelectable key={photo.id} src={`${import.meta.env.VITE_IMAGES_URL}/${photo.imageId}`} title={photo.title} imageClassName="w-20 h-20" onSelectImage={(selected) => handleTogglePhoto(selected, photo.id)} />)}
                            </div>
                        }
